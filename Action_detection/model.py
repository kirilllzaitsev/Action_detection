...
class BBoxRegressor:
    ...
    def gen_boxes(self, x:torch.tensor) -> (torch.tensor, torch.tensor):
        ...
        bboxes = self.fc_bbox(bboxes).reshape(9,4)
...
class TPN:
    ...
    def update_boxes(self, bboxes:torch.tensor, conv2:torch.tensor):
        ...
        :tensor bboxes: bboxes from conv5 (9x4); 4 -> H, W, x, y
        ...
        scaled_bboxes = bboxes.detach().clone().numpy()
        # rescale bounding boxes
        scaled_bboxes[:, 0] *= 200 / 19  
        scaled_bboxes[:, 3] *= 200 / 19  
        scaled_bboxes[:, 1] *= 150 / 19  
        scaled_bboxes[:, 2] *= 150 / 19  
        scaled_bboxes = scaled_bboxes.astype(np.int64)
#         scaled_bboxes.resize(150,200,9,4, refcheck=False)
        
        # slice out tubes from conv2 feature map
        tubes = conv2.data.numpy()[:, :, :,
